...
Forward.prototype.tick = function(){
    ...
    this.dx = rotation.cos * ds;
    this.dy = rotation.sin * ds;
    location.x += this.dx;
    location.y += this.dy;
};

Forward.prototype.checkCollisions = function( impactors ){
    var i , w , h , s , impactor ,
        animation = this.animation ,
        impacted = [];

    for( i = 0 ; i < impactors.length ; i++ ){
        impactor = impactors[i];
        if( animation === impactor ) continue;
        w = animation.location.x - impactor.location.x;
        h = animation.location.y - impactor.location.y;
        s = Math.sqrt( w * w , h * h );

        if( s < ( animation.radius + impactor.radius ) )
            impacted.push( impactor );
    }
    
    if( impacted.length > 0 )
        this.emit( 'collision' , impacted );
};
...
Cannon.prototype.fire = function(){
    ...
    if( bomb ){
        radius += bomb.animation.radius;
        bomb.setLocation( this.location.x + radius * rotation.cos , this.location.y + radius * rotation.sin );
...
Cannon.prototype.tick = function(){
    ...
    if( this.loaded.length < this.size &&
        ...
        bomb.on( 'collision' , function( impacted ){
