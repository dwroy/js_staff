...
Util.inherits( Bomb , Forward , new Forward( new Animation ) );
...
var Chamber = function( bombClass , bombImage , v , maxRange , size , loadTime ){
    Fx.call( this );
...
Util.inherits( Chamber , Fx );

Chamber.prototype.start = function(){
    if( this.state === Fx.STATE_STOP )
        this.state = Fx.STATE_RUN;
};

Chamber.prototype.stop = function(){
    if( this.state === Fx.STATE_RUN )
        this.state = Fx.STATE_STOP;
};
...
Chamber.prototype.fire = function(){
    ...
    if( bomb ){
        ...
        bomb.setRotation( r.radian , r.cos , r.sin );
        bomb.owner = this;
        bomb.start();
    }
};

Chamber.prototype.attach = function( scene ){
    var bomb , 
        i = 0 ,
        updated = [] ,
        fired = this.fired;
...
Chamber.prototype.tick = function(){
    var bomb;
