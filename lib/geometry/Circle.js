...
Circle.prototype = {
    ...
    isIntersected: function( geometry ){
        ...
        if( geometry instanceof Rectangle )
            return this.isIntersectedInRectangle( geometry );
    ...
    isIntersectedInRectangle: function( rectangle ){
        var center = this.center,
            ...
            w = Math.abs( center.x - rectangleCenter.x ),
            h = Math.abs( center.y - rectangleCenter.y );

        if( w > this.radius + rectangle.halfWidth ) return false;
        if( h > this.radius + rectangle.halfHeight ) return false;

        w -= rectangle.halfWidth;
        h -= rectangle.halfHeight;

        if( w * w + h * h > this.radius * this.radius ) return false;
