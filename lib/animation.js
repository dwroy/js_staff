...
var Move = function( sceneImage , velocity ){
    ...
    this.coordinate = sceneImage.geometry.coordinate;
...
Move.prototype.tick = function( sceneImages ){
    var coordinate = this.coordinate ,
        sameDepthImages = sceneImages[ coordinate.z ] ,
        circle = this.sceneImage.geometry;
    
    for( var i = 0 ; i < sameDepthImages.length ; i++ ){
        if( circle !== sameDepthImages[i].geometry )
        circle.isIntersected( sameDepthImages[i] );
    }
};
